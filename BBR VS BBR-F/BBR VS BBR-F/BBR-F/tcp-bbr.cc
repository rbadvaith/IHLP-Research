...
TcpBbr::TcpBbr(void) :
  ...
  NS_LOG_INFO(this << "  BBR-F version: v" << bbr::VERSION);
...
// Copy BBR-F congestion control algorithm across socket.
Ptr<TcpCongestionOps> TcpBbr::Fork() {
  NS_LOG_FUNCTION(this);
  return CopyObject<TcpBbr> (this);
}

// BBR-F ignores calls to increase window.
...
// BBR-F does not use ssthresh, so ignored.
...
void TcpBbr::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t packets_acked,
  ...
  // BW ESTIMATION
  // Based on [CCYJ17b]:
  // Cheng et al., "Delivery Rate Estimation", 
  //
  //                                (outstanding)
...
// Events/calculations specific to BBR-F congestion state.
// tcb = transmission control block
